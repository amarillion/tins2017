...
{
private:
	map<IComponentPtr, IComponentPtr> popupParentMap;
public:
	RootComponent()
	{
	...
	void addWithParent(IComponentPtr child, IComponentPtr parent)
	{
		add(child);
		popupParentMap[child] = parent;
	}

	virtual void handleMessage(IComponentPtr src, int msg) override
	{
...
MainLoop::MainLoop() :
		...
		stretch (false), smokeTest(false), logicIntervalMsec(20),
...
//		allegro_message("Unable initialize graphics module\n%s\n", allegro_error); //TODO
	...
	else
	{
		...
		setDimension(al_get_display_width(display), al_get_display_height(display));
	}
...
void MainLoop::popup (IComponentPtr popupWindow, IComponentPtr parent)
{
	if (parent)
	{
	...
	rootPane->repr (0, std::cout); /* TODO remove dbg statment */
}
